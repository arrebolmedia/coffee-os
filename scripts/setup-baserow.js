  ...
  locations: {
    ...
    fields: [
      ...
      { name: 'email', type: 'email' },
      {
        name: 'timezone',
        type: 'single_select',
        select_options: [{ value: 'America/Mexico_City', color: 'blue' }],
      },
...
class BaserowConfigurator {
  ...
  async init() {
    ...
    try {
      ...
      console.log(
        `📊 Base de datos: ${DATABASE_NAME} (ID: ${this.databaseId})`,
      );
      console.log(
        `🗂️ Tablas creadas: ${this.createdTables}/${this.totalTables}`,
      );
      console.log('\n🎯 Próximos pasos:');
  ...
  async createDatabase() {
    ...
    try {
      ...
      console.log(
        `✅ Base de datos creada: ${DATABASE_NAME} (ID: ${this.databaseId})`,
      );
    } catch (error) {
  ...
  async createTables() {
    ...
    for (const [tableKey, tableConfig] of Object.entries(TABLES_SCHEMA)) {
      try {
        ...
        const fieldsResponse = await baserowClient.get(
        ...
        this.createdTables++;
        console.log(
          `   ✅ Tabla completada: ${tableConfig.name} (${this.createdTables}/${this.totalTables})`,
        );
        ...
        console.error(
          `   ❌ Error creando tabla ${tableConfig.name}:`,
          error.message,
        );
        throw error;
      }
  ...
  async createField(tableId, fieldConfig, tableKey) {
    ...
    switch (fieldConfig.type) {
      ...
      case 'link_row':
        if (
          fieldConfig.link_row_table &&
          this.tableIds[fieldConfig.link_row_table]
        ) {
          fieldData.link_row_table_id =
            this.tableIds[fieldConfig.link_row_table];
        } else {
          // Si la tabla referenciada no existe aún, saltamos este campo
          console.log(
            `   ⏳ Postergando campo de relación: ${fieldConfig.name}`,
          );
          return;
        }
        break;
        ...
        if (fieldConfig.type === 'link_row' && fieldConfig.link_row_table) {
          ...
          if (targetTableId) {
            try {
              ...
              console.log(
                `   ✅ Relación creada: ${tableConfig.name}.${fieldConfig.name}`,
              );
            } catch (error) {
  ...
  async importInitialData() {
      ...
      for (const roleData of rolesData) {
        try {
          await baserowClient.post(
            `/database/tables/${rolesTableId}/rows/`,
            roleData,
          );
        } catch (error) {
          // Ignorar duplicados
    ...
    if (categoriesTableId) {
      for (const categoryData of categoriesData) {
        try {
          await baserowClient.post(
            `/database/tables/${categoriesTableId}/rows/`,
            categoryData,
          );
        } catch (error) {
          // Ignorar duplicados
