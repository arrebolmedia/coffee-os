...
export class TicketsService {
  ...
  async void(ticketId: string, reason: string, user: AuthUser): Promise<Ticket> {
  ...
  private calculateTotals(lines: CreateTicketLineDto[]) {
    ...
    for (const line of lines) {
      ...
      const modifiersTotal = line.modifiers?.reduce((sum, m) => sum + m.priceDelta, 0) || 0
...
export const usePOSStore = create<POSStore>((set, get) => ({
  ...
  addItem: (product, modifiers = []) => {
    ...
    if (existingItem) {
      ...
      const subtotal = product.price + modifiersWithDelta.reduce((sum, m) => sum + m.priceDelta, 0)
  ...
  get subtotal() {
    return get().cart.reduce((sum, item) => sum + item.subtotal * item.quantity, 0)
  },
      ...
      try {
        ...
        switch (item.type) {
          case 'CREATE_TICKET':
            ...
            break
          case 'ADD_PAYMENT':
            result = await api.post(`/tickets/${item.payload.ticketId}/payments`, item.payload)
            break
          case 'VOID_TICKET':
            result = await api.post(`/tickets/${item.payload.ticketId}/void`, item.payload)
            break
        }
