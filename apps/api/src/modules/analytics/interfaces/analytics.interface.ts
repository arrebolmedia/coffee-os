...
export interface SalesMetrics {
  ...
  // Comparison. `*_change_percent` is null when the previous-period value
  // was zero and the current one is positive (not comparable).
  vs_previous_period?: {
    gross_sales_change: number;
    gross_sales_change_percent: number | null;
    orders_change: number;
    orders_change_percent: number | null;
    avg_order_value_change: number;
    avg_order_value_change_percent: number | null;
  };
}
...
export interface ProductPerformance {
  ...
  /** true when product.cost is missing → profit/margin are computed as if cost=0 */
