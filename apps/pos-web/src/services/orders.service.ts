...
class OrdersService {
  ...
  async getOrderByNumber(orderNumber: string, organizationId: string): Promise<Order> {
  ...
  async updateOrderStatus(id: string, status: Order['status']): Promise<Order> {
    const response = await apiClient.patch<Order>(`${this.baseUrl}/${id}/status`, { status });
    return response.data;
  }
  ...
  async addPayment(
    ...
    const response = await apiClient.post<Order>(`${this.baseUrl}/${orderId}/payments`, data);
    return response.data;
  }

  async refundOrder(orderId: string, amount?: number, reason?: string): Promise<Order> {
    const response = await apiClient.post<Order>(`${this.baseUrl}/${orderId}/refund`, {
      amount,
      reason,
    });
    return response.data;
  ...
  async getDailySales(organizationId: string, date: Date): Promise<{
    total_sales: number;
    total_orders: number;
  ...
  async getMonthlySales(organizationId: string, year: number, month: number): Promise<any> {
    const response = await apiClient.get<any>(
